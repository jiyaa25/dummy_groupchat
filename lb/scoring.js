 ...
 * - Avg Response Time: 0-2000ms mapped to 0-100
 */
function normalizeMetrics(queueLength, cpu, memory, activeRequests, avgResponseTime) {
    const q = Math.max(0, queueLength || 0);
    const c = Math.max(0, Math.min(100, cpu || 0));
    const m = Math.max(0, Math.min(100, memory || 0));
    const a = Math.max(0, activeRequests || 0);
    const rt = Math.max(0, avgResponseTime || 0);

    const normQueue = Math.min(100, q * 10);
    const normCpu = c;
    const normMem = m;
    const normActive = Math.min(100, a * 10);
    const normRt = Math.min(100, (rt / 2000) * 100);
 ...
 * @param {number} effectiveQueue Effective in-flight queue
 * @param {number} effectiveActive Effective active requests
 * @returns {{ score: number, breakdown: object }}
 */
function calculateScore(backend, effectiveQueue, effectiveActive) {
    const q = effectiveQueue !== undefined ? effectiveQueue : (backend.queueLength || 0);
    const a = effectiveActive !== undefined ? effectiveActive : (backend.activeRequests || 0);
    const c = backend.cpu || 0;
    const m = backend.memory || 0;
    const rt = backend.avgResponseTime || 0;

    const norm = normalizeMetrics(q, c, m, a, rt);

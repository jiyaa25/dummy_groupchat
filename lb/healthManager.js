...
class HealthManager {
    constructor(backendManager) {
        ...
        this.timeoutMs = parseInt(process.env.HEARTBEAT_TIMEOUT_MS || "3500", 10);
        ...
        try {
            ...
            const req = client.request(urlObj, {
                ...
                if (res.statusCode === 200) {
                    const wasUnhealthy = !backend.healthy;
                    backend.healthy = true;
                    backend.lastHeartbeat = Date.now();
                    backend.consecutiveFailures = 0;
                    if (wasUnhealthy) console.log(`[HealthManager] Backend ${backend.id} recovered and marked HEALTHY`);
                } else {
    ...
    recordFailure(backend, errorMsg) {
        ...
        if (backend.healthy && backend.consecutiveFailures >= 3) {
            backend.healthy = false;
            console.warn(`[HealthManager] Backend ${backend.id} marked UNHEALTHY after ${backend.consecutiveFailures} consecutive failures (${errorMsg})`);
        }
    }
